	...
	public static void main(String[] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps * increment + start; numToCheck += increment) {
			...
			for (int i = 0; i < trials; i++) {
				...
				double bdb = bs.getFleschScore();
			}
			Long endTimeB = System.nanoTime();
			double expTimeB = (endTimeB - startTimeB) / 1000000000.0;
			System.out.print(expTimeB + "\t");
			// EfficientDocument
			Long startTimeE = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				EfficientDocument es = new EfficientDocument(str);
				double edb = es.getFleschScore();
			}
			Long endTimeE = System.nanoTime();
			double expTimeE = (endTimeE - startTimeE) / 1000000000.0;
			System.out.print(expTimeE + "\n");
